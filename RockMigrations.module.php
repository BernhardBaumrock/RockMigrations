...
class RockMigrations extends WireData implements Module {
   ...
   * 
   * <code>
   * $rm->addTranslationsToLanguage("https://github.com/apeisa/Finnish-ProcessWire/archive/master.zip", "finnish");
   * </code> 
   * 
  ...
  public function setTranslationsToLanguage(string $urlToTranslationsZip, string $languageName = "default") {
    ...
    if(count($items)) {
      ...
      foreach($items as $item) {
        ...
        $language->language_files->add(config()->paths->cache . $item);
      }
    }
    $language->save();
